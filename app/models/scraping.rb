...
class Scraping < ApplicationRecord
  ...
  # 指定リンクにGETリクエスト
  # エラーの場合、エラーメッセージ出力
  def self.get_request(agent, link)
  ...
  # 指定XPathのリンクを配列で取得
  def self.get_child_links(page, xpath)
    links      = []
    links_text = []
    if page
      elements = page / xpath
      ...
      links_2[0].each_with_index do |link_2, index_links_2|
        ...
        links_3[0].each_with_index do |link_3, index_links_3|
          ...
          if page
            ...
            # chord = Chord.new
            song_chord = Chord.where(song: song, artist: artist).first_or_initialize
            
            # 楽曲コード譜ページURLを取得
            song_chord.page_url = page.uri.to_s
            
            puts "#{DateTime.now.minute}:#{DateTime.now.second}"
            
            chords = page / XPATH_LINK_3_CHORDS
            chords = page / XPATH_LINK_3_CHORDS_2 if chords.empty?
            if chords.empty?
              puts "\n\n\n******************\n\n\nコード無い\n#{link_3}\n\n\n******************\n\n\n"
            end
            puts song_chord.chords = chords.inner_text
            
            song_chords << song_chord
          end
        end
        Chord.import(song_chords, on_duplicate_key_update: %i(song artist page_url chords))
      end
    end
  end
end
